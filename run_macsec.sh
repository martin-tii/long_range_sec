...
configure_mac_sec()
{
  ...
  then
    ...
    echo $(ip macsec add macsec0 tx sa 0 pn 1 on key 01 "$key1")
    ip macsec add macsec0 rx port 1 address "$mac" sa 0 pn 1 on key 01 "$key2"
  else
    ip macsec add macsec0 tx sa 0 pn 1 on key 01 "$key2"
    ip macsec add macsec0 rx port 1 address "$mac" sa 0 pn 1 on key 02 "$key1"
  fi
